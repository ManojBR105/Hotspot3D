...
std::string outfile = "/net/home/mba151/hotspot3D_HLS_yxz/0_baseline/output_512x8.out";
...
int main(int argc, char** argv)
{
    ...
    kernel_3D_hw(powerIn, temp_hw, answer_hw, Cap, Rx, Ry, Rz, dt, amb_temp);
    ...
    // float acc_sw = accuracy(tempOut,answer_sw,numRows*numCols*layers);
    // printf("OMP Time: %.3f (s)\n",time);
    printf("FPGA Time: %.3f (s)\n",time1);
    printf("CPU Time: %.3f (s)\n",time2);
    accuracy(temp_sw,temp_hw,numRows*numCols*layers);
    // printf("OMP Accuracy: %e\n",acc_sw);
    // printf("Error: %e\n",acc_hw);
