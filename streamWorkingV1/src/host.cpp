        ...
        for(y = 0; y < ny; y++)
            for(x = 0; x < nx; x++)
                for(z = 0; z < nz; z++)
                {
                    ...
                    tOut[c] = ((tIn[c]*cc + (dt/Cap) * pIn[c]) + cw*(tIn[w]+tIn[e])) + (cn*(tIn[n]+tIn[s]) + ct*((tIn[t]+tIn[b]) + amb_temp));
                }
        ...
        for(y = 0; y < ny; y++)
            for(x = 0; x < nx; x++)
                for(z = 0; z < nz; z++)
                {
                    ...
                    tIn[c] = ((tOut[c]*cc + (dt/Cap) * pIn[c]) + cw*(tOut[w]+tOut[e])) + (cn*(tOut[n]+tOut[s]) + ct*((tOut[t]+tOut[b]) + amb_temp));
                }
        // float *temp = tIn;
        // tIn = tOut;
        // tOut = temp; 
