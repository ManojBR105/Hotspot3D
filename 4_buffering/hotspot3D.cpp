...
void kernel_3D_hw(WD_t* powerIn, WD_t* tempIn, WD_t* tempOut, float Cap, float Rx, float Ry, float Rz, float dt, float amb_temp)
{
  ...
  WD_t local_tin [2][ITY][TX];
  WD_t local_pin [2][ITY][TX];
  WD_t local_tout [2][TY][TX];
  ...
  #pragma HLS array_partition variable=local_tin complete dim=1
  #pragma HLS array_partition variable=local_pin complete dim=1
  #pragma HLS array_partition variable=local_pin complete dim=1

  main_i0:for(int i = 0; i < ITR/2/STAGES; i++) {
    main_j0:for (int j = 0; j<(NY/TY)+2; j++) {
      switch(j%2){
      case 0:
        load0(tempIn, local_tin[0], j);
        load1(powerIn, local_pin[0], j);
        workload(local_pin[1], local_tin[1], local_tout[1], cc, ce, cn, ct, stepDivCap, amb_temp);
        store(tempOut, local_tout[0], j-2);
        break;
      case 1:
        load0(tempIn, local_tin[1], j);
        load1(powerIn, local_pin[1], j);
        workload(local_pin[0], local_tin[0], local_tout[0], cc, ce, cn, ct, stepDivCap, amb_temp);
        store(tempOut, local_tout[1], j-2);
        break;
      }
    }
  }
  main_i1:for(int i = 0; i < ITR/2/STAGES; i++) {
    main_j1:for (int j = 0; j<(NY/TY)+2; j++) {
      switch(j%2){
      case 0:
        load0(tempOut, local_tin[0], j);
        load1(powerIn, local_pin[0], j);
        store(tempIn, local_tout[0], j-2);
        workload(local_pin[1], local_tin[1], local_tout[1], cc, ce, cn, ct, stepDivCap, amb_temp);
        break;
      case 1:
        load0(tempOut, local_tin[1], j);
        load1(powerIn, local_pin[1], j);
        workload(local_pin[0], local_tin[0], local_tout[0], cc, ce, cn, ct, stepDivCap, amb_temp);
        store(tempIn, local_tout[1], j-2);
        break;
      }
    }
  }
